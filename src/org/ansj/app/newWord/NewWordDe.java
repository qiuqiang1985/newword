...
public class NewWordDe {
	 ...
	 * @return
	 */
	public static String removePeople(String content){
	
		StringBuilder sb = new StringBuilder();
		boolean isPeople = false;
		for(int i=0;i<content.length();i++){
			if(content.charAt(i)=='@'){ //表示人名的开始
				isPeople = true;
			}
	...
	public static void main(String[] args) throws IOException {
		String content = "E人E本可以说是评测中心的老朋友了，经历了三代升级，在年末的时候我们拿到了第四代产品——E人E本T4，熟悉E人E本的朋友都知道，这款平板电脑定位于商务应用，与市售大多数娱乐平板有着本质的区别，凭借原笔迹数字书写技术，E人E本获得了国内高端政商用户的认可，作为即将投放在2012年的新品，T4有着怎样的表现呢？";
		...
		// content ="这个能威慑犯罪分子么？这车能用来镇压群体事件么？虽然你们不是第一次不把纳税人的钱当成钱，但是也不要这么......";

		// content="要继续学习本文中的内容，我假定您已经安装了 Python 2.5 或者更高版本，因为本文中的许多示例都将使用 Python 语言的新特性，而这些特性仅出现于 Python2.5 之后。要开始使用 Python 语言的线程，我们将从简单的 Hello World 示例开始：";
		content = "忠犬八公，把我看哭了";
...
//		}
//		
//		System.out.println(removePeople(content));
//		
//		System.exit(0);
		
		
		BufferedReader reader = new BufferedReader(new FileReader(
				"/home/qiuqiang/more.xml"));

		BufferedWriter writer = new BufferedWriter(new FileWriter(
				"/home/qiuqiang/weibo_nw.txt"));
		
		BufferedWriter writer1 = new BufferedWriter(new FileWriter(
				"/home/qiuqiang/weibo.txt"));
		...
		try {
			while ((line = reader.readLine()) != null) {
				...
				if (line.startsWith("<article>")) {
					...
					try{
						content = removePeople(line.substring("<article>".length(),
								line.length() - "</article>".length()));
					...
					List<Term> parser = NlpAnalysis.parse(
							StringUtil.rmHtmlTag(content), learn);
					HashSet<String> resultKeys = new HashSet<String>();
					for (Term term : parser) {
						resultKeys.add(term.getName());
					}

//					 System.out.println(parser);

					List<Entry<String, Double>> top = learn.getTopTree(1000);
					if (top == null) {
						System.out.println("没有发现新词");
					} else {
						for (Entry<String, Double> entry : top) {
							if (resultKeys.contains(entry.getKey())) {
								writer.write(entry + "\n");
							}
						}
					}
